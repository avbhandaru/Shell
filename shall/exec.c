...
// BEGIN
    if (fcntl(fd1, F_GETFD) < 0 || fcntl(fd2, F_GETFD) < 0) {
        printf("File descriptor(s) are invalid.\nExiting.");
        _exit(1);
    }
    int fd_new = dup2(fd2, fd1);
    if (fd_new == -1) _exit(1);
// END
}
...
// BEGIN
    int fd2 = open(name, flags, 0644); //0o644
    if (fcntl(fd, F_GETFD) < 0 || fcntl(fd2, F_GETFD) < 0) {
        close(fd2);
        printf("Closed file due to invalidity.");
        _exit(1);
    }
    int fd_new = dup2(fd2, fd);
    if (fd_new == -1) _exit(1);
    close(fd2);
// END
}
...
// BEGIN
    ...
    if (child_pid == 0) {
        ...
        int pid_fork = wait(&status);
        
        while (child_pid != pid_fork) {
            // If wait (pid_fork) is the child then we can exit loop
            if (WIFSIGNALED(status)) {
                printf("Process %d process terminated by signal\n", pid_fork);
                printf("Exit status %d\n", WEXITSTATUS(status));
            } else {
                printf("Process %d terminated with exit status %d\n", pid_fork, WEXITSTATUS(status));
            }
            
            pid_fork = wait(&status);
        }
        
        if (WIFSIGNALED(status)) {
            printf("Process %d process terminated by signal %d\n", pid_fork, WTERMSIG(status));
        }
